...
def _build_statistics_loader(batch_size: int, num_workers: int):
...
def _build_datasets_and_loaders(batch_size: int, num_workers: int, mean_val: float, std_val: float):
    dataset_base_path = config.DATASET_BASE_PATH_BREADM
    image_type = "VIBRANT+C2"
...
def train_breadm_baselines():
    ...
    _section("Preparing normalization statistics")
    _info("Building statistics dataloader")
    statistics_loader = _build_statistics_loader(batch_size=batch_size, num_workers=num_workers)
    _info("Computing dataset mean/std")
