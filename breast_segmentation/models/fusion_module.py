...
class BreastFusionModel(L.LightningModule):
    ...
    def forward(self, image1: torch.Tensor, image2: torch.Tensor, image3: torch.Tensor) -> torch.Tensor:
        """Forward pass with 3 inputs."""
        mask = self.model(image1, image2, image3)
        return mask

    def step(self, batch: Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]], batch_idx: int, stage: str) -> torch.Tensor:
    ...
    def single_predict_sliding_window(self, image1, image2, image3, roi_size=(128, 128), sw_batch_size=128, overlap=0):
