...
def create_patient_datasets(
    patient_ids: List[str],
    dataset_base_path: str,
    mean_patches_sub,
    std_patches_sub,
    mean_no_thorax_third_sub,
    std_no_thorax_third_sub,
    num_workers: int,
    ...
    test_transforms_patches_sub = Compose(
        [
            LoadImaged(
                keys=["image", "label"], image_only=image_only, reader=monai.data.NumpyReader()
            ),
    ...
    for patient_id in patient_ids:
        ...
        no_thorax_sub_test_ds = CacheDataset(
            data=test_dicts, transform=test_transforms_no_thorax_third_sub, num_workers=num_workers
        )
        patches_sub_test_ds = CacheDataset(
            data=test_dicts, transform=test_transforms_patches_sub, num_workers=num_workers
        )
...
def test_dataset_aware_fusion(
    ...
    for patient_id in patient_ids:
        ...
        fusion_dataset = PairedDataset(whole_image_ds, patches_ds, augment=False, filter_data=True)
...
def test_dataset_aware_ensemble(
    ...
    for patient_id in patient_ids:
        ...
        for idx, e in tqdm(enumerate(patches_ds), total=len(patches_ds)):
            ...
            # SECOND MODEL
            if (
                fusion_dataset[idx][0]["keep_sample"]
                or fusion_dataset[idx][1]["keep_sample"]
                or fusion_dataset[idx][2]["keep_sample"]
            ):
    ...
    print(
        f"Standard Deviation of Memory Usage: {std_memory_usage / (1024**2):.2f} MB"
    )  # Convert to MB
