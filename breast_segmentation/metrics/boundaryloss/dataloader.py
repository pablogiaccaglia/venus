...
def _img_to_numpy_with_newaxis(img):
    return np.array(img)[np.newaxis, ...]


def _normalize_to_unit(nd):
    return nd / 255


def _numpy_to_float_tensor(nd):
    return torch.tensor(nd, dtype=torch.float32)


def _npy_to_numpy_with_newaxis(npy):
    return np.array(npy)[np.newaxis, ...]


def _npy_to_numpy(npy):
    return np.array(npy)


def _to_tensor(nd):
    return torch.tensor(nd)


def _to_float_tensor(nd):
    return torch.tensor(nd, dtype=torch.float32)
...
def _to_int64_tensor_with_batch(nd):
    return torch.tensor(nd, dtype=torch.int64)[None, ...]
...
def _tensor_to_cpu_numpy(t):
    return t.cpu().numpy()


def _random_shuffle_sample(x):
...
def png_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose(
        [
            ...
            _img_to_numpy_with_newaxis,
            _normalize_to_unit,  # max <= 1
            _numpy_to_float_tensor,
        ]
    )


def _equalize_image(img):
...
def npy_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([_npy_to_numpy_with_newaxis, _numpy_to_float_tensor])


def raw_npy_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([_npy_to_numpy, _numpy_to_float_tensor])


def from_numpy_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([_to_tensor])


def tensor_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([_numpy_to_float_tensor])
...
def gt_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose(
        [
            _img_to_numpy,
            _to_int64_tensor_with_batch,  # Add one dimension to simulate batch
...
def get_loaders(
    ...
    for i, (train_topfolder, folders_list) in enumerate(
        ...
        train_names: list[str] = map_(_path_to_name, train_folders[0].glob("*"))
        ...
        train_set = gen_dataset(train_names, train_folders, spacing_dict=train_spacing_dict)
        if args.group_train:
        ...
        if i == args.val_loader_id or (
            ...
            val_names: list[str] = map_(_path_to_name, val_folders[0].glob("*"))
            ...
            val_set = gen_dataset(val_names, val_folders, spacing_dict=val_spacing_dict)
            val_sampler = (
                PatientSampler(val_set, args.grp_regex, shuffle=False) if args.group else None
            )
            val_batch_size = 1 if val_sampler else batch_size
            val_loader = data_loader(val_set, batch_sampler=val_sampler, batch_size=val_batch_size)
...
class SliceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[
        str,
        Union[str, int, Tensor, list[Tensor], list[Tuple[slice, ...]], list[Tuple[Tensor, Tensor]]],
    ]:
        ...
        t_tensors: list[Tensor] = [
            tr(resolution, self.K)(e) for (tr, e) in zip(self.transforms, images)
        ]
...
class PatientSampler(Sampler):
    def __init__(self, dataset: SliceDataset, grp_regex, shuffle=False, quiet=False) -> None:
        ...
        self.shuffle_fn: Callable = _random_shuffle_sample if self.shuffle else id_
        ...
        # Create sorting key function for this specific context
        def _sorting_key_func(i):
            return filenames[i]

        for pid in self.idx_map.keys():
            self.idx_map[pid] = sorted(self.idx_map[pid], key=_sorting_key_func)
