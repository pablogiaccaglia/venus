...
class AsymmetricFocalLoss(nn.Module):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        back_ce = (1 - self.delta) * (1 - y_true) * torch.pow(y_pred, self.gamma) * torch.log(1 - y_pred + 1e-7)
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def __init__(self, delta: float = 0.7, gamma: float = 0.75, alpha: float = 0.5, beta: float = 0.5):
        super().__init__()
        self.delta = delta
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        back_dice = (1 - self.delta) * torch.sum((1 - y_true) * y_pred ** 2, dim=(2, 3)) / \
                    (torch.sum((1 - y_true) * y_pred ** 2 + (1 - y_true) * (1 - y_pred) ** 2, dim=(2, 3)) + 1e-7)
...
class FocalTverskyIOULoss(nn.Module):
    ...
    def __init__(self, alpha: float = 0.5, beta: float = 0.5, gamma: float = 0.75, iou_weight: float = 0.5):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
...
class CABFL(nn.Module):
    ...
    def __init__(self, idc: List[int], weight_aufl: float = 0.5, delta: float = 0.6, gamma: float = 0.2):
        super().__init__()
    ...
    def forward(self, logits: torch.Tensor, probs: torch.Tensor, dist_maps: torch.Tensor, 
                gts: torch.Tensor, current_epoch: int) -> torch.Tensor:
        """
        Args:
            logits: Predicted logits [B, C, H, W]
            probs: Predicted probabilities [B, C, H, W] 
...
def get_loss_function(loss_name: str, **kwargs) -> nn.Module:
    ...
    if loss_name == 'dice':
        return MonaiDiceLoss(sigmoid=True, **kwargs)
