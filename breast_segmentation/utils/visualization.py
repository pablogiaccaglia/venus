...
from breast_segmentation.utils.inference import (
    get_image_label_files_patient_aware,
    reverse_transformations,
)


def plot_slices_side_by_side(
...
def visualize_predictions(
    ...
    patient_images, patient_labels = get_image_label_files_patient_aware(
        dataset_base_path, split, image_type, patient_id
    )

    if slice_idx >= len(patient_images):
        print(
            f"Slice index {slice_idx} out of range for patient {patient_id} (max: {len(patient_images)-1})"
        )
        return
    ...
    if model_type == "venus":
        ...
        model = BreastSegmentationModel.load_from_checkpoint(
            model_path, arch_name=arch_name, strict=False, map_location=device
        )
