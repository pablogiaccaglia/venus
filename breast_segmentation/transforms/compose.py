...
from monai.transforms import (
    ...
    HistogramNormalized, MedianSmooth, Rotate90d,
    DeleteItemsd, ScaleIntensityd
)
...
from .preprocessing import (
    ...
    ThresholdBlack, MedianSmooth as MedianSmoothTransform,AdaptiveCropBreasts, BoundingBoxSplit
)
...
class PrepareSample(MapTransform):
    ...
    def __init__(self, target_size, subtracted_images, patches, **kwargs):
        ...
        self.resize = Resized(keys=['image', 'label'], spatial_size=target_size, mode='nearest-exact')
        self.patches = patches
        self.resize_original = Resized(keys=['original_image', 'original_label'], spatial_size=target_size, mode='nearest-exact')
        self.subtracted_images = subtracted_images
        self.loadimage = monai.transforms.LoadImage(ensure_channel_first=True, reader=monai.data.PILReader(converter=pil_grayscale_converter))
    ...
    def prepare_with_patches(self, data):
        ...
        if data['has_mask']:
            ...
            # CRITICAL: Apply trim_breast_coords for patches
            start, end = trim_breast_coords
...
class Preprocess(MapTransform):
    ...
    def __init__(self,
        ...
        self.median_smooth = MedianSmoothTransform(keys=['processed_image'], radius=median_smooth_radius)
        self.histogram_normalized = HistogramNormalized(keys=['processed_image'], num_bins = hist_norm_num_bins)
        self.remove_thorax = RemoveThorax(keys=['processed_image', 'processed_label'], threshold= rm_thorax_threshold, margin=rm_thorax_margin, value=0)
        self.remove_bottom = RemoveBottom(keys=['processed_image', 'processed_label'], threshold = rm_bottom_threshold,margin=rm_bottom_margin, value=0)
        ...
        self.crop = CropToSquare(keys=['processed_image','processed_label'], shrink_factor=15, black_threshold=400)
        self.resizePreliminary = Resize(step='preliminary', keys=['image', 'label'], spatial_size=target_size_preliminary)
        self.resizePreliminaryCleanImg = Resized(keys=['image', 'label'],spatial_size=target_size_preliminary, mode='nearest-exact')
    ...
    def __call__(self, data: Dict[str, Any]) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        if self.get_patches:
            ...
            data[0]['image_meta_dict']['original_channel_dim'] = MetaTensor(data[0]['image_meta_dict']['original_channel_dim'])
            data[0]['label_meta_dict']['original_channel_dim'] = MetaTensor(data[0]['label_meta_dict']['original_channel_dim'])
    
            data[1]['image_meta_dict']['original_channel_dim'] = MetaTensor(data[1]['image_meta_dict']['original_channel_dim'])
            data[1]['label_meta_dict']['original_channel_dim'] = MetaTensor(data[1]['label_meta_dict']['original_channel_dim'])
            ...
            data['image_meta_dict']['original_channel_dim'] = MetaTensor(data['image_meta_dict']['original_channel_dim'])
            data['label_meta_dict']['original_channel_dim'] = MetaTensor(data['label_meta_dict']['original_channel_dim'])
